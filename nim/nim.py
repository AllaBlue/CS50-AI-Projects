...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        if (tuple(state), action) in self.q.keys():
            ...
            self.q[tuple(state), action] = self.alpha * (new_value_estimate - old_q)       
    ...
    def best_future_reward(self, state):
        ...
        for action in actions:
            if (tuple(state), action) in self.q.keys():
                rewards.add(self.q[tuple(state), action])
            else:
                rewards.add(0)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in actions:
            ...
            q_value_action.add((q_value, action))

        best_action = sorted(q_value_action, key=lambda x: x[0], reverse=True)[0][1]
        ...
        if not epsilon:
            ...
            for action in actions:
                if action == best_action:
                    ...
                    weights.append(self.epsilon/len(actions))
